		...
		&& isset($_REQUEST['app']) && !empty($_REQUEST['app']))
{
	...
	$sql_select_app_id = "select * from ".$db.".tbl_application where app_ApplicationName in (".$res_team_name.") ";
	...
	//echo $sql_select_prev_data; exit;
...
?>
		<tr>
			...
			<td>
				...
				year(pts_ReleaseDate) = '".$year."' and pts_ApplicationName in (".$res_team_name.") order by pts_ReleaseDate ";
				//echo $sql_select_release_dt; exit;
				?>
				<select id="row[0][ddl_rel_date][]" name="row[0][ddl_rel_date][]"
					style="border-radius: 8px;width: 140px;" onchange="getPRNumber(this,document.getElementById('ddl_app').value)" >
...
if(isset($_REQUEST['res_dt']) && !empty($_REQUEST['res_dt'])
		&& isset($_REQUEST['team']) && !empty($_REQUEST['team']))
		{
			...
			$sql_select_pr_num = "select distinct(pts_ProjectNum) from " . $db . ".tbl_ptsdata where app_SlNo in (" . $res_team_name . ")
